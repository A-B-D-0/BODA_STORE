document.addEventListener('DOMContentLoaded', function() {
    ...
    function displayProducts(products) {
        ...
        products.forEach(product => {
            ...
            let buttonHTML = `<button class="disabled-button" onclick="addToCart(${product.id})"><i class="fa-solid fa-cart-shopping"></i>`;
            let outOfStockMessage = '';

        // إذا كان السعر "stock"، عطّل زر الشراء وأضف فئة CSS مختلفة
        if (product.price === '') {
            buttonHTML = `<button class="disabled-button" disabled><i class="fa-solid fa-cart-shopping"></i></button>`;
            outOfStockMessage = `<div class="out-of-stock-message">OUT OF STOCK.</div>`;
        }
    ...
    document.getElementById('share-vodafone').addEventListener('click', function() {
        ...
        let cartDetails = ` MOBILE LEGENDS\n نورت الاستور يا : (${customerName})\n\n :الايدى(ID): ${customerId}\n\n: معرف الاعب : ${additionalInfo}`;
        ...
        if (!customerId || !additionalInfo) {
            alert('   الرجاء كتابه الايدى ومعرف اللاعب(ID)');
            return;
        }
